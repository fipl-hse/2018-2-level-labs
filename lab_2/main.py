...
def propose_candidates(word: str, max_depth_permutations: int=1) -> list:
    if word is None or word == '' or max_depth_permutations != 1:
        return([])
    #if word != '' or max_depth_permutations == 1 or word != None:
    for index in range(1,len(word)):
        candidate_list.append(word[:index] + word[index + 1:])  # list.append(new_word) #удаление одной буквы из слова
           #candidate_list.append(word[:index-1] + word[index] + word[index-1] + word[index+1:])
