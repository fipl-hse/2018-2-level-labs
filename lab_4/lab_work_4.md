...
Требуется сделать очистку текста от ненужных симоволов и знаков пункктуации
...
### Шаг 2.7 Расчет расстояния между представлениями текстов
...
Идея такая, что тексты на одну и ту же тему будут иметь схожее векторное
представление.
...
3. Найдем [косинусное](https://en.wikipedia.org/wiki/Cosine_similarity) 
