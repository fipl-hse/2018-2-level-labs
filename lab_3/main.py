...
class NGramTrie:
    ...
    def fill_from_sentence(self, sentence):
        ...
        right = self.size
        left = 0
        for tokens in range(len(sentence)):
            chaotic_grams.append(tuple(sentence[left:right]))
            right = right + 1
            left = left + 1
            if right > len(sentence):
    ...
    def predict_next_sentence(self, prefix):
        ...
        for k, value in candidates.items():
            if value >= candidates.get(al_eq):
                best_grams.append(k)
        perf_gram = list(sorted(best_grams)[0])
        return perf_gram


def generate_text(ngr, number_sentences, prefix):
    best_grams = []
    for sentence in encode(ws.from_corpus(split_by_sentence(REFERENCE_TEXT)), split_by_sentence(REFERENCE_TEXT)):
        ngr.fill_from_sentence(sentence)
    ngr.calculate_log_probabilities()
    pre_grams = ngr.predict_next_sentence(prefix)
    for i in range(number_sentences):
        al_eq = max(pre_grams.items(), key=operator.itemgetter(1))[0]
        for k, value in pre_grams.items():
            if value >= pre_grams.get(al_eq):
                best_grams.append(k)
        perf_gram = sorted(best_grams)[0]
        best_grams.pop(best_grams.index(perf_gram))
        decoded_grams = [ws.get_original_by(i) for i in perf_gram]
        if best_grams is []:
            pass
        return decoded_grams
